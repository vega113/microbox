...
opt.appName = '&mu;box' //set the app name
...
function create_reply_box(indented){ //REMEMBER TO REMOVE INFO ONCE GOOGLE FIXES THE BUG
  ...
  if(!opt.no_sig) box.textbox.value = '\n\nPosted with ' + constants.app_url + ' ' +addonsig;
...
function logoff_ui(){
  loading(false);
	...
	var txt = getEl('login_button').innerHTML.replace('@example.com','@' + constants.wiab_domain);
	getEl('login_button').innerHTML = txt;
}
    ...
    add.onclick = function(){
      ...
      if(participant){
        if(participant.indexOf("@") == -1){
          participant += "@" + constants.wiab_domain;
        }
...
//    wave_container.appendChild(tags);
    ...
    var wiab_url = constants.wiab_domain + ':' + constants.wiab_port;
    footer.innerHTML = '<a href="http://' + wiab_url + '#'+waveId+'" target="_blank">Open this wave in the official Wave in a Box client</a>';
...
function startup() {
	if (onLine()) {
		wave.robot.notifyCapabilitiesHash(); // switch to
		// l83s7 v3rz10n
		getUsername(); // get the username of the user
		// TODO: something to pull username from cache if
		// offline?
	}
	if (onLine() == false) {
...
constants.wiab_domain='vegalabz.com';
